...
const LargeScreen: FunctionComponent = () => {
  ...
  return (
    <div className="relative bg-white w-full h-full overflow-hidden text-center text-[1rem] text-white font-oxanium">
      ...
      <div onClick={toggleAudio} className={`z-20 mr-4 mb-4 absolute bottom-0 right-0 bg-nightsky/70  rounded-full w-fit h-fit border-2 border-${state == "success" ? "border-green-400" : state == "error" ? "border-red-600" : `border-${color}`} transition-all shadow-2xl shadow-${state == "success" ? "shadow-green-400" : state == "error" ? "shadow-red-600" : `shadow-${color}`} `}>
      ...
      <div className="absolute top-0 left-0">
        ...
        <Fade triggerOnce cascade>
          ...
          <div className="mx-auto z-10 bg-nightsky/70 w-3/4">
            ...
            <b className="font-oxanium-semibold inline-block text-[3.4rem]  min-w-full  italic text-center ">
              THE NEXT GENERATION
            </b>
            <b className="font-oxanium-bold inline-block text-[3.4rem]  min-w-full  -mt-4 italic text-center ">
              OF GAMING CREATORS
            </b>
            <div className={`font-oxanium-extrabold transition-all inline-block  text-[8.4rem] ${state == "success" ? "text-green-400" : state == "error" ? "text-red-600" : `text-${color}`} font-extrabold  -mt-8 min-w-full  text-center italic `}>
              ARE HERE
            </div>
            <b className="font-oxanium-semibold inline-block text-[2.8rem]  -mt-6 min-w-full  italic text-center">
        ...
        <div className={`mx-auto mt-8  shadow-2xl shadow-${state == "success" ? "shadow-green-400" : state == "error" ? "shadow-red-600" : `shadow-${color}`}`}>
          <form className="mx-auto" onSubmit={subscribeUser}>
            <div className={`transition-all mx-auto bg-transparent rounded-lg box-border w-3/4 h-[3.88rem] border-[2px] border-solid ${state == "success" ? "border-green-400" : state == "error" ? "border-red-600" : `border-${color}`} shadow-lg ${state == "success" ? "shadow-green-400" : state == "error" ? "shadow-red-600" : `shadow-${color}`}`}>
            ...
            <div className="mt-4 mx-auto text-[2.5rem] font-extrabold my-auto  w-[60rem] h-[3.19rem]">
        ...
        <div className="mt-12">
          <p className={`font-oxanium-semibold uppercase italic bg-nightsky/70 rounded-2xl mx-auto w-fit p-2 border-2 border-${state == "success" ? "border-green-400" : state == "error" ? "border-red-600" : `border-${color}`} `}>WHY GG? CLICK AN ITEM!</p>
          <div className="-mt-4">
        ...
        <div className={`mt-4 mx-auto rounded-lg box-border w-3/4 h-fit border-[2px] border-solid border-${color} bg-nightsky/70`}>
